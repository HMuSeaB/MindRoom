...
- **🔊 生成式音效引擎 (Generative Audio)**: 
  - **Pure Tone**: 摒弃采样，完全由正弦波实时合成，纯净无噪
  - **Adaptive Decay**: 慢速输入时产生如寺钟般的长混响，快速输入时自动收紧为清脆的冰裂声
- **🎵 有机可视化 (Organic Visualizer)**:
  - **Cymatics**: 声音在粒子场中激发出真实的“克拉尼图形”冲击波
  - **Resonance**: 粒子随音量共振发光，呈现自然的聚散与流动
  - **Living Particles**: 具有物理边界与生命周期的粒子系统，即便被打散也会自动回归
- **🍂 仪式化交互**:
